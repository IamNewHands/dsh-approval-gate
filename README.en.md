...
- 🩺 **Session-event access fixed at the root** (v0.8.4): the plugin always read `session.events`, but DSH's `Session` exposes only `snapshotEvents()` / `ownEvents()` — so structured arguments, the deterministic hard-deny layer, and the judge's user-authorization source were all dead. It now reads `snapshotEvents() → ownEvents() → events`. **Behaviour change**: calls writing into `C:\Windows\...`, `~/.ssh` and other protected locations are hard-rejected with no dialog, and the dangerous-keyword layer can see the real command, so keyword hits escalate to a human more often
- 🧹 **No phantom command line** (v0.8.5): only command-flavoured tools (`pwsh`, `bash`, `terminal-bash`, …) print a command line; `write` / `edit` show just the target path, so a file approval never carries a meaningless `命令：host未提供`
- 📊 **Approval explanations become a field table** (v0.9.0): each approval record renders five fields — **operation type / target path / blast radius / command / model note**. The operation type is decided at command level (`Remove-Item`/`rm`/`format`/`git reset --hard` → deletion, `git … push`/`gh release`/`npm publish`/`scp` → push/publish, `write` → create/write, `edit` → modify), the blast radius states the consequence of the sandbox mode (whole machine / workspace / read-only), and values are colour-coded: deletion red, create green, modify blue, push amber. The host card's plain text is now one field per line (`操作：… / 路径：… / 影响：… / 命令：… / 原因：…`) instead of a prose paragraph, and the notice strip shows a one-line summary. Events carry a new `facts` field (older events get them derived by the events API from what was recorded, with nothing written back); events without `facts` still render from text
- 🔢 **The "审批" tab shows a count** (v0.9.0): the tab reads `审批 (12)` — the number of records the view actually lists for this session (pending approvals live only in the notice strip and are excluded) — so you can tell at a glance whether anything new arrived without opening the tab

...
DeepSeek Harness 自动审批门控插件 v0.7.0：**最小人工介入，只把必须人工确认的操作转人工（fail-safe）**。
...
1. **提示条**（输入框上方独立一行，`conversation.input.dock` order=30，不随流式对话滚动）：
   ...
   - **静默拒绝 / 人工拒绝 → 红色**（v0.7.0+）：显示「已直接拒绝：<操作>」或「已拒绝：<操作>」，**不自动收起**——一直挂在对话框上方，直到你切到「审批」tab 或按下处理按钮。提示条右侧提供：
...
3. **追认（「重新审批通过」，v0.7.0+）**：被拒绝的记录可以追认，效果是**写入带操作指纹的自动放行规则 + 向会话投递一条重试指令**（让 AI 重跑那个操作，不必你手敲一遍）。两道围栏：
   ...
   - 重复追认幂等（规则不重复写入）；追认后该记录标注「已追认」并从待处理计数中移除
...
>
> `reconsidered` 是**追认记录**（v0.7.0+）：`reconsiderOf` 指回被追认的原事件。事件 API 会过滤掉追认记录本身，并给原事件加上 `reconsidered: true`，前端据此标注「已追认」并撤销待处理角标。

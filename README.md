...
判定管道：**硬拒（凭据 / 系统路径，判定模型无权推翻）→ 硬事实人工 → 危险词 → 白名单 → denyRules → 脱敏 → 判定（JSON allow/ask/deny）→ 学习沉淀**。
...
- 🛡️ **硬风险永远人工**：删除、凭据、远程/生产、系统路径、批量不可回补五类操作直接转人工，不计数、不学习；判定模型给出 `allow` 也强制转人工
- 🎯 **确认制学习**：同一操作人工确认 N 次后，第 N+1 次起自动放行；沉淀规则携带**操作指纹**，只放行你确认过的操作
- 🧠 **语义同类验证**：措辞变化但意图相同的操作，由判定模型对照你的确认样本语义判断，不再依赖关键词
- ♻️ **判定器失败兜底**（吸收）：连续失败前 2 次静默拒绝让 agent 改方案，第 3 次转一次人工；成功一次即清零
...
| 连续失败计数 | 按会话计数，前 2 次静默拒绝、第 3 次转人工，成功即清零；阈值 `judgeFailureLimit`（默认 3） |
